<x-layouts.app :title="'Order placed | Vehicle Battery Store'">
    <div class="mx-auto max-w-2xl">
        <x-card padding="p-8">
            <div class="text-center">
                ...
                <h1 class="mt-4 text-2xl font-bold text-ink-900">Order placed!</h1>
                <p class="mt-1 text-sm text-ink-600">Thanks for your order. We've sent a confirmation email to <strong>{{ $order->billing_email }}</strong>.</p>
            </div>
            ...
            <dl class="mt-6 grid gap-4 rounded-lg bg-ink-50 p-4 text-sm sm:grid-cols-2">
                ...
                <div>
                    <dt class="text-xs uppercase text-ink-500">Total amount</dt>
                    <dd class="mt-0.5 font-semibold text-ink-900">₹{{ number_format((float) $order->total, 2) }}</dd>
                </div>
                <div>
                    <dt class="text-xs uppercase text-ink-500">Payment method</dt>
                    <dd class="mt-0.5 font-semibold text-ink-900">{{ strtoupper($order->payment_method) }}</dd>
                </div>
                <div>
